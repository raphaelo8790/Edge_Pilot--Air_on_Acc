...
| `package.json` | Dependencies and the 30 npm scripts. `prebuild` and `pretest` regenerate the vision fixtures, so a build and a test run both start from a known dataset |
