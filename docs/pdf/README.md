...
Generated from the `FOLDER.md` files and `00-PROJECT-RECORD.md` at the project
root. Same content, paginated for reading away from a screen with the source
tree open.
...
These are build output, not sources. If a `FOLDER.md` changes, the PDF beside
it is stale until it is regenerated - the markdown is always authoritative.
